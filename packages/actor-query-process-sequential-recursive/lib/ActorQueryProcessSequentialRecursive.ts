...
export class ActorQueryProcessSequentialRecursive extends ActorQueryProcessSequential {
  ...
  public override async parse(query: QueryFormatType, context: IActionContext): Promise<IQueryProcessSequentialOutput> {
    ...
    const initializedKey = new ActionContextKey<boolean>('initialized');
    const physicalQueryPlanLoggerKey = new ActionContextKey<() => IPhysicalQueryPlanLogger>('physicalQueryPlanLogger');
    if (!context.get(initializedKey)) {
      if (context.get(physicalQueryPlanLoggerKey)) {
        const factory = context.getSafe(physicalQueryPlanLoggerKey);
