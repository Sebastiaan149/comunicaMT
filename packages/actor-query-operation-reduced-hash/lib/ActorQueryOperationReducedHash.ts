...
export class ActorQueryOperationReducedHash extends ActorQueryOperationTypedMediated<Algebra.Reduced> {
  ...
  public async runOperation(
    ...
    const variables = (await output.metadata()).variables.map(
      (v) => v.variable,
    );
    const bindingsStream: BindingsStream = output.bindingsStream.filter(
      await this.newHashFilter(context, variables),
    );
    return {
      type: "bindings",
      bindingsStream,
  ...
  public async newHashFilter(
    ...
    const { hashFunction } = await this.mediatorHashBindings.mediate({
      context,
    });
